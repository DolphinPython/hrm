...
while ($row = $leave_days_result->fetch_assoc()) {
    // echo "Leave ID : ".$row['leave_type_id'];
    // echo " -|- ";
    // echo "No of Days : ".$row['no_of_days'];
    // echo " -|- ";
    // echo "Status : ".$row['status'];
    // echo "<br>";

    if ($row['no_of_days'] == 0.25 && $row['status'] == 2) {
        $leaveshort += 0.25;
    } else if ($row['no_of_days'] == 0.5 && $row['status'] == 2) {
        $leavehalf += 0.5;
    } else if ($row['no_of_days'] == 1 && $row['status'] == 2) {
        $leavefull += 1;
    }
}
