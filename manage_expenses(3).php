...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['expense_form'])) {
    try {
        ...
        if ($expense_id > 0) {
            $sql = "UPDATE employee_expenses SET employee_id=?, expense_date=?, category_id=?, amount=?, description=?";
            $params = [$employee_id, $expense_date, $category_id, $amount, $description];
            $types = "isids";
            ...
            $sql = "INSERT INTO employee_expenses (employee_id, expense_date, category_id, amount, description, receipt_path, status, submitted_at) VALUES (?, ?, ?, ?, ?, ?, 'Pending', NOW())";
            $stmt = $con->prepare($sql);
...
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    try {
        ...
        if ($result->num_rows == 1) {
            ...
            $alert = "<div class='alert alert-warning'>Expense not found or not printable.</div>";
        }
        $stmt->close();
    } catch (Exception $e) {
...
$monthly_totals = [];
$employee_totals = [];
$employee_monthly_totals = [];

try {
    $sql = "SELECT ee.*, CONCAT(e.fname, ' ', e.lname) AS employee_name,
            DATE_FORMAT(ee.expense_date, '%Y-%m') AS month
            FROM employee_expenses ee
            JOIN hrm_employee e ON ee.employee_id = e.id
            WHERE 1=1 $status_query $employee_query $month_query $week_query $date_query";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $employee_totals[$employee] = ($employee_totals[$employee] ?? 0) + $row['amount'];
        $employee_monthly_totals[$employee][$month] = ($employee_monthly_totals[$employee][$month] ?? 0) + $row['amount'];
    }
} catch (Exception $e) {
    $alert = "<div class='alert alert-danger'>Error calculating summaries: " . htmlspecialchars($e->getMessage()) . "</div>";
}

// Fetch categories for the manage categories modal
$categories = [];
try {
...
// Get employee name for filter if applicable
$employee_name = '';
if ($employee_filter) {
    try {
            ...
            <div class="container py-4">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-12 col-md-4">
                        <div class="card card-shadow p-3">
                            <h6>Employee Totals</h6>
                            <ul class="list-group list-group-flush">
                                <?php foreach ($employee_totals as $employee => $amount): ?>
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                ...
                <?php if (!empty($employee_monthly_totals)): ?>
                    <div class="card card-shadow p-3 mb-4">
                        <h6>Employee Monthly Breakdown</h6>
                        <div class="accordion" id="employeeMonthlyAccordion">
                            <?php foreach ($employee_monthly_totals as $employee => $months): ?>
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="heading<?= md5($employee) ?>">
                                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= md5($employee) ?>" aria-expanded="false" aria-controls="collapse<?= md5($employee) ?>">
                                            <?= htmlspecialchars($employee) ?>
                                        </button>
                                    </h2>
                                    <div id="collapse<?= md5($employee) ?>" class="accordion-collapse collapse" aria-labelledby="heading<?= md5($employee) ?>" data-bs-parent="#employeeMonthlyAccordion">
                ...
                <div class="card card-shadow p-3">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered align-middle" id="expenseTable">
                            ...
                            <?php
                            try {
                                $sql = "SELECT ee.*, CONCAT(e.fname, ' ', e.lname) AS employee_name, c.name AS category_name
                                        FROM employee_expenses ee
                                        JOIN hrm_employee e ON ee.employee_id = e.id
                                        LEFT JOIN expense_categories c ON ee.category_id = c.id
                                        WHERE 1=1 $status_query $employee_query $month_query $week_query $date_query
                                ...
                                echo "<tr><td colspan='8' class='text-center text-danger'>Error loading expenses: " . htmlspecialchars($e->getMessage()) . "</td></tr>";
                            }
                            ?>
                            </tbody>
                        </table>
...
function generatePDF() {
    try {
        ...
        if (employeeName) {
            filterText = `${employeeName} (${filterText})`;
        }

        const expenses = [];
<?php
try {
    $sql = "SELECT ee.*, c.name AS category_name
            FROM employee_expenses ee
            JOIN hrm_employee e ON ee.employee_id = e.id
            LEFT JOIN expense_categories c ON ee.category_id = c.id
            WHERE 1=1 $status_query $employee_query $month_query $week_query $date_query
...
?>
        ...
        companyDetails.innerHTML = `
            ...
            <p style="margin: 0;">No-51-52, Delhi, Delhi-110092</p>
            <p style="margin: 0;">CIN: U74999DL2016PTC307712</p>
        `;
        ...
        table.innerHTML = `
            <thead>
                <tr>
                    <th style="border: 1px solid #000; padding: 8px; background-color: #B2EBF2;">Category</th>
                    <th style="border: 1px solid #000; padding: 8px; background-color: #B2EBF2;">Date</th>
                    <th style="border: 1px solid #000; padding: 8px; background-color: #B2EBF2;">Description</th>
                    <th style="border: 1px solid #000; padding: 8px; background-color: #B2EBF2;">Invested Rupees</th>
                </tr>
            </thead>
            <tbody>
                ${expenses.length === 0 ? '<tr><td colspan="4" style="border: 1px solid #000; padding: 8px; text-align: center;">No expenses found.</td></tr>' : 
                expenses.map(exp => `
                    <tr>
                        <td style="border: 1px solid #000; padding: 8px;">${exp.category}</td>
                        <td style="border: 1px solid #000; padding: 8px;">${exp.expense_date}</td>
                        <td style="border: 1px solid #000; padding: 8px;">${exp.description}</td>
                        <td style="border: 1px solid #000; padding: 8px; text-align: right;">${exp.amount}</td>
                    </tr>
                `).join('')}
            </tbody>
        `;
        ...
        totals.innerHTML = `
            <table style="width: 40%; border-collapse: collapse;">
                <tr>
                    <td style="padding: 8px; font-weight: bold; background-color: #006064; color: white;">Total</td>
                    <td style="padding: 8px; text-align: right; background-color: #006064; color: white;">${total}</td>
                </tr>
            </table>
        `;
