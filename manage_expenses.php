...
<div class="card card-shadow p-3 mb-4">
    <h5 class="mb-3 badge bg-warning">Employee Expense Breakdown by Category</h5>
    <div class="accordion" id="employeeCategoryAccordion">
        <?php
        try {
            // Build the WHERE clause dynamically
            $where_clauses_custom = [];
            $params_custom = [];
            $types_custom = "";

            if ($status_filter) {
                $where_clauses_custom[] = "ee.status = ?";
                $params_custom[] = $status_filter;
                $types_custom .= "s";
            }
            if ($from_date && $to_date) {
                $where_clauses_custom[] = "ee.expense_date BETWEEN ? AND ?";
                $params_custom[] = $from_date;
                $params_custom[] = $to_date;
                $types_custom .= "ss";
            }
            if ($week_filter && in_array($week_filter, ['1', '2', '3', '4', '5'])) {
                // Since month_filter is removed, week filter needs a base date
                $base_date = new DateTime(); // Use current date as fallback
                $week_start_custom = clone $base_date;
                $week_start_custom->modify("-" . $base_date->format('N') . " days"); // Start of current week
                $week_start_custom->modify("+ " . (($week_filter - 1) * 7) . " days");
                $week_end_custom = clone $week_start_custom;
                $week_end_custom->modify("+6 days");
                $where_clauses_custom[] = "ee.expense_date BETWEEN ? AND ?";
                $params_custom[] = $week_start_custom->format('Y-m-d');
                $params_custom[] = $week_end_custom->format('Y-m-d');
                $types_custom .= "ss";
            }

            // Construct WHERE clause
            $where_clause_custom = !empty($where_clauses_custom) ? "WHERE " . implode(" AND ", $where_clauses_custom) : "";

            // Log filter variables for debugging
            error_log("Filters: status=$status_filter, week=$week_filter, from=$from_date, to=$to_date");

            // Fetch employees with expenses
            $employee_query_custom = "SELECT DISTINCT e.id, CONCAT(e.fname, ' ', e.lname) AS employee_name
                              FROM hrm_employee e
                              JOIN employee_expenses ee ON e.id = ee.employee_id
                              $where_clause_custom
                              ORDER BY employee_name";

            // Log the query and parameters
            error_log("Employee Query: $employee_query_custom");
            error_log("Params: " . print_r($params_custom, true));

            $stmt_custom = $con->prepare($employee_query_custom);
            if (!$stmt_custom) {
                throw new Exception("Prepare failed: " . $con->error);
            }
            if (!empty($params_custom)) {
                $stmt_custom->bind_param($types_custom, ...$params_custom);
            }
            if (!$stmt_custom->execute()) {
                throw new Exception("Execute failed: " . $stmt_custom->error);
            }
            $employee_result_custom = $stmt_custom->get_result();

            if ($employee_result_custom->num_rows === 0) {
                echo "<p class='text-center'>No employee expenses found.</p>";
            }

            while ($employee_custom = $employee_result_custom->fetch_assoc()) {
                $emp_id_custom = $employee_custom['id'];
                $emp_name_custom = htmlspecialchars($employee_custom['employee_name']);
                $accordion_id_custom = md5($emp_name_custom);

                // Fetch category-wise expenses for this employee
                $category_query_custom = "SELECT ec.name AS category_name, SUM(ee.amount) AS total_amount
                                  FROM employee_expenses ee
                                  LEFT JOIN expense_categories ec ON ee.category_id = ec.id
                                  WHERE ee.employee_id = ? $where_clause_custom
                                  GROUP BY ec.id, ec.name
                                  ORDER BY ec.name";
                error_log("Category Query for Employee $emp_id_custom: $category_query_custom");
                $stmt_cat_custom = $con->prepare($category_query_custom);
                if (!$stmt_cat_custom) {
                    throw new Exception("Category prepare failed: " . $con->error);
                }
                $cat_params_custom = array_merge([$emp_id_custom], $params_custom);
                $cat_types_custom = "i" . $types_custom;
                if (!empty($cat_params_custom)) {
                    $stmt_cat_custom->bind_param($cat_types_custom, ...$cat_params_custom);
                }
                if (!$stmt_cat_custom->execute()) {
                    throw new Exception("Category execute failed: " . $stmt_cat_custom->error);
                }
                $category_result_custom = $stmt_cat_custom->get_result();

                // Calculate remaining amount
                $advance_query_custom = "SELECT SUM(ee.amount) AS advance_total
                                 FROM employee_expenses ee
                                 JOIN expense_categories ec ON ee.category_id = ec.id
                                 WHERE ee.employee_id = ? AND ec.name = 'ADVANCE DISBURSEMENT AMOUNT' $where_clause_custom";
                error_log("Advance Query for Employee $emp_id_custom: $advance_query_custom");
                $stmt_adv_custom = $con->prepare($advance_query_custom);
                if (!$stmt_adv_custom) {
                    throw new Exception("Advance prepare failed: " . $con->error);
                }
                $adv_params_custom = array_merge([$emp_id_custom], $params_custom);
                if (!empty($adv_params_custom)) {
                    $stmt_adv_custom->bind_param($cat_types_custom, ...$adv_params_custom);
                }
                if (!$stmt_adv_custom->execute()) {
                    throw new Exception("Advance execute failed: " . $stmt_adv_custom->error);
                }
                $advance_result_custom = $stmt_adv_custom->get_result();
                $advance_total_custom = $advance_result_custom->fetch_assoc()['advance_total'] ?? 0;

                $expense_query_custom = "SELECT SUM(ee.amount) AS expense_total
                                 FROM employee_expenses ee
                                 JOIN expense_categories ec ON ee.category_id = ec.id
                                 WHERE ee.employee_id = ? AND ec.name != 'ADVANCE DISBURSEMENT AMOUNT' $where_clause_custom";
                error_log("Expense Query for Employee $emp_id_custom: $expense_query_custom");
                $stmt_exp_custom = $con->prepare($expense_query_custom);
                if (!$stmt_exp_custom) {
                    throw new Exception("Expense prepare failed: " . $con->error);
                }
                if (!empty($adv_params_custom)) {
                    $stmt_exp_custom->bind_param($cat_types_custom, ...$adv_params_custom);
                }
                if (!$stmt_exp_custom->execute()) {
                    throw new Exception("Expense execute failed: " . $stmt_exp_custom->error);
                }
                $expense_result_custom = $stmt_exp_custom->get_result();
                $expense_total_custom = $expense_result_custom->fetch_assoc()['expense_total'] ?? 0;

                $remaining_amount_custom = $advance_total_custom - $expense_total_custom;
                $remaining_class_custom = $remaining_amount_custom >= 0 ? 'bg-success' : 'bg-danger';
                ?>
                <div class="accordion-item">
                    <h2 class="accordion-header" id="headingEmp<?= $accordion_id_custom ?>">
                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                data-bs-target="#collapseEmp<?= $accordion_id_custom ?>" aria-expanded="false"
                                aria-controls="collapseEmp<?= $accordion_id_custom ?>">
                            <?= $emp_name_custom ?> (Remaining: ₹ <?= number_format($remaining_amount_custom, 2) ?>)
                        </button>
                    </h2>
                    <div id="collapseEmp<?= $accordion_id_custom ?>" class="accordion-collapse collapse"
                         aria-labelledby="headingEmp<?= $accordion_id_custom ?>" data-bs-parent="#employeeCategoryAccordion">
                        <div class="accordion-body">
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <?php
                                        if ($category_result_custom->num_rows === 0) {
                                            echo "<tr><td colspan='2' class='text-center'>No expenses found for this employee.</td></tr>";
                                        }
                                        while ($cat_row_custom = $category_result_custom->fetch_assoc()) {
                                            echo "<tr>
                                                <td>" . htmlspecialchars($cat_row_custom['category_name'] ?? '-') . "</td>
                                                <td>₹ " . number_format($cat_row_custom['total_amount'], 2) . "</td>
                                            </tr>";
                                        }
                                        ?>
                                    </tbody>
                                    <tfoot>
                                        <tr>
                                            ...
                                            <td>₹ <?= number_format($advance_total_custom, 2) ?></td>
                                        </tr>
                                        <tr>
                                            <th>Total Expenses (Excl. Advance)</th>
                                            <td>₹ <?= number_format($expense_total_custom, 2) ?></td>
                                        </tr>
                                        <tr>
                                            <th>Remaining Amount</th>
                                            <td><span class="badge <?= $remaining_class_custom ?>">₹ <?= number_format($remaining_amount_custom, 2) ?></span></td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
                ...
                <?php
                $stmt_cat_custom->close();
                $stmt_adv_custom->close();
                $stmt_exp_custom->close();
            }
            $stmt_custom->close();
        } catch (Exception $e) {
            echo "<div class='alert alert-danger'>Error loading employee breakdown: " . htmlspecialchars($e->getMessage()) . "</div>";
            error_log("Employee Breakdown Error: " . $e->getMessage());
        }
        ?>
    </div>
</div>
