...
// Capture POST data
$ipAddress = "random number";  // Replace with actual dynamic IP logic
...
try {
    ...
    if ($officeTimingsResult && mysqli_num_rows($officeTimingsResult) > 0) {
        ...
        $evening_half_time = strtotime($officeTimingRow['evening_half_time'] ?? "03:00 PM");
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        if ($record['clock_in_time']) {
            ...
            // New condition: Evening Half Day status
            $lateStatus = '';
            $statusColor = '';
            ...
            $updateSql = "
                ...
                SET clock_out_time = '$currentTime', clock_out_ip = '$ipAddress', status = 'logout', 
        ...
        $insertSql = "
            ...
            VALUES ($userId, '$currentTime', '$ipAddress', 'login', '$currentTime', '$lateStatus', '$statusColor')
        ";
