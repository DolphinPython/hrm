...
{
	//email
    $to = "pythondolphin@gmail.com, dolphinpython@outlook.com";
...
function remaining_leave($leave_type_id, $emp_session_id)
{
    ...
    // Step 1: Fetch the total allocated leave days for the leave type
    $query_allocated = "SELECT number_of_leave FROM hrm_leave_type WHERE id = '$leave_type_id';";
    $result_allocated = mysqli_query($conn, $query_allocated) or die(mysqli_error($conn));

    if (mysqli_num_rows($result_allocated) > 0) {
        $row_allocated = mysqli_fetch_array($result_allocated);
        $allocated_days = $row_allocated['number_of_leave']; // Total allocated leave days
    } else {
        // If no allocated days are found, assume 0
        $allocated_days = 0;
    }

    // Step 2: Calculate the total leave days taken by the employee for this leave type
    $query_taken = "SELECT SUM(no_of_days) AS total_taken FROM hrm_leave_applied 
                    WHERE emp_id = '$emp_session_id' AND leave_type_id = '$leave_type_id';";
    $result_taken = mysqli_query($conn, $query_taken) or die(mysqli_error($conn));

    if (mysqli_num_rows($result_taken) > 0) {
        $row_taken = mysqli_fetch_array($result_taken);
        $total_taken = $row_taken['total_taken']; // Total leave days taken
    } else {
        // If no leave days are taken, assume 0
        $total_taken = 0;
    }

    // Step 3: Calculate the remaining leave days
    $remaining_leave = $allocated_days - $total_taken;
...
function get_value($table, $column, $id)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_array($result);
        return $row[$column]; // Return the column value
    } else {
...
// 	while ($row1 = mysqli_fetch_array($result1)) {
// 		$x++;
// 		$reporting_manager_id = $row1['reporting_manager_id'];
// 		$query2 = "SELECT * FROM hrm_employee WHERE id = '$reporting_manager_id';";
// 		$result2 = mysqli_query($conn, $query2) or die(mysqli_error($conn));
		
// 		$row2 = mysqli_fetch_array($result2);
// 		if ($row2) { // ✅ Only process if result is valid
// 			if ($x % 1 == 0) $br = "<br><br>";
...
// 					</div>
// 				</div>
// 				<a href="profile.php?id='.$row2['id'].'">'.$row2['fname'].' '.$row2['lname'].'</a>
// 			</div>'.$br;
// 		}
// 	}
// 	return $row_reporting_manager_name;
