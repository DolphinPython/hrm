...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    $relaxation_late = $row['relaxation_late'];
    $monthly_shorts = $row['monthly_shorts'];
    $monthly_half = $row['monthly_half'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $relaxation_late = mysqli_real_escape_string($conn, $_POST['relaxation_late']);
    $monthly_shorts = mysqli_real_escape_string($conn, $_POST['monthly_shorts']);
    $monthly_half = mysqli_real_escape_string($conn, $_POST['monthly_half']);
    $monthly_leaves = mysqli_real_escape_string($conn, $_POST['monthly_leaves']);
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        $updateQuery = "
            ...
            SET 
                ...
                relaxation_late = '$relaxation_late',
                monthly_shorts = '$monthly_shorts',
                monthly_half = '$monthly_half',
        ...
        $insertQuery = "
            INSERT INTO office_timing (
                relaxation_time, relaxation_late, monthly_shorts, monthly_half, monthly_leaves, normal_fine, extra_fine_time, extra_fine, half_day_time, evening_half_time, login_time, logout_time, saturday_option
            ) VALUES (
                '$relaxation_time', '$relaxation_late', '$monthly_shorts', '$monthly_half', '$monthly_leaves', $normal_fine, '$extra_fine_time', $extra_fine, '$half_day_time', '$evening_half_time', '$login_time', '$logout_time', '$saturday_option'
            )";
