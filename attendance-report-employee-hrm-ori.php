                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function () {
                                ...
                                for (let d = 1; d <= daysInMonth; d++) {
                                    ...
                                    let cls = 'day';
                                    let label = '';
                                    if (present.includes(dateStr)) { cls += ' present'; label = 'Present'; }
                                    else if (leaves.includes(dateStr)) { cls += ' leave'; label = 'Leave'; }
                                    else if (holidays.includes(dateStr)) { cls += ' holiday'; label = 'Holiday'; }
                                    else {
                                        const thisDate = new Date(year, month - 1, d);
                                        if (thisDate < today) { cls += ' absent'; label = 'Absent'; }
                                        else { cls += ' future'; label = ''; }
                                        const dayOfWeek = thisDate.getDay();
                                        if (dayOfWeek === 6) { cls += ' saturday'; label = 'Saturday'; }
                                        if (dayOfWeek === 0) { cls += ' sunday'; label = 'Sunday'; }
                                    }
